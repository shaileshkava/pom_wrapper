...
public class RemoveAll extends BaseClass{
  ...
  @Test
  public void RemoveList(String testName) throws IOException {
	  ...
	  try{
		  ...
		  extent.log(LogStatus.PASS, "List added successfully!");
		 
		  if(!bVisible(pm.getListBoxPage().btnRemAll, getDriver(), 5))
			  throw new ProjException("Remove button is not visible###removeAllMissing");
		  
		  extent.log(LogStatus.PASS, "Clicking on remove all button");
		  pm.getListBoxPage().btnRemAll.click();
		  
		  bVisible(pm.getListBoxPage().listAvailableItems.get(1), getDriver(), 5);
		  
		  //Failed deliberate
		  if(pm.getListBoxPage().listRemovedItems.size() > 0)
			  throw new ProjException("Remove button is not visible###removeAllMissing");
