...
try{
		  ...
		  pdf.bVisible(pdf.strEnteredMessage, 10);
		  
		  new Ashot_Screenshot().captureScreenshot("test1", getDriver(), pdf.userInput);
		  extent.log(LogStatus.PASS, "Messaged entered as expected", extent.addScreenCapture(pathScreenshot+"test1.png"));
		  
		  if(strEnteredMessage.equalsIgnoreCase(data)){
			  new Ashot_Screenshot().captureScreenshot("test1", getDriver(), pdf.userInput);
			  extent.log(LogStatus.FAIL, "Message could not enter properly", extent.addScreenCapture(pathScreenshot+"test1.png"));  
		  }
